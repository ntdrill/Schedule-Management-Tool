...
### 1.4 用語注記：`03_バージョン別機能スコープ.md §6` との関係

**注記（用語衝突の明示）**:

`docs/readme/03_バージョン別機能スコープ.md` §6「開発フェーズ (v1)」には Phase 1〜5（Project Setup / Sensor Logic / Data Model / Watch UI / iOS UI）が記載されています。これは **v1 アプリ実装の粗いマイルストーン**を示すものであり、**本書 §3〜§4 で扱う Agent Workflow Phase 1〜8（Foundation 〜 Validation）および §5 の Loop 1〜3 とは別軸の概念**です。

混同防止のため、本書では以下のように呼び分けます：

| 体系 | 番号と名称 | 出典 | 対象 |
|:---|:---|:---|:---|
| **Agent Workflow Phase** | Phase 1: Foundation 〜 Phase 8: Validation | `Agent_Workflow_Queue/v1_phase1.txt` 他、`phase3以降のphase.txt` | 設計〜実装パイプライン（本書の主題） |
| **Loop** | Loop 1: リセットと定義確立 〜 Loop 3: 記号抽出と設計 | `completed_init_v1_alignment.txt` | v1 移行プロセス（本書 §5） |
| **v1 開発マイルストーン** | Phase 1: Project Setup 〜 Phase 5: iOS UI | `docs/readme/03_バージョン別機能スコープ.md` §6 | v1 アプリ実装の粗い区分（本書では扱わない） |

リポジトリ実態（`Agent_workspace/Agent_maneger/data/Agent_Workflow_Queue/v1_phase1.txt` 〜 `v1_phase6.txt` の存在、`Agent_workspace/Agent_maneger/クロック受付/` 下の `Phase6_DeviceAssignment_完了` / `Phase7_Prototype_完了` クロック）は **本書の Agent Workflow Phase 体系**と一致します。一方、03 §6 の Phase 1〜5 は v1 アプリの実装ステップを別軸で示したもので、両者の Phase 番号は対応関係を持ちません。
