...
- [x] chore(quality): SwiftLint の `identifier_name` を再有効化（`min_length: { warning: 2, error: 1 }`, `max_length: { warning: 50, error: 80 }` 程度に緩和）し、`HealthKitService.swift` の `let ms = ...` を `let milliseconds = ...` にリネーム。`.swiftlint.yml` の disabled_rules から `identifier_name` を外す。CI が緑のまま維持されることを確認（push 後 `/loop` cron が CI 結果を確認する） — 2026-04-28 01:30 JST commit:9bee95f
- [x] docs(readme): リポジトリルートの README（または無ければ `docs/readme/00_README索引.md`）に `iOS Build & Test` workflow の status badge を追加。マークダウン形式で `https://github.com/ntdrill/Schedule-Management-Tool/actions/workflows/ios-build.yml/badge.svg?branch=sdk-migration` を埋め込み、Actions 一覧へのリンクも併記する — 2026-04-28 02:30 JST commit:9bee95f
