...
- [x] chore(coverage): CI workflow の test ステップに `-enableCodeCoverage YES -resultBundlePath TestResults.xcresult` を付与。後続ステップで `xcrun xccov view --report --json TestResults.xcresult > coverage.json` を生成し、`actions/upload-artifact` で `TestResults.xcresult` と `coverage.json` を CI 成果物として残す。閾値判定は導入しない（最初は可視化のみ） — 2026-04-27 23:31 JST commit:<pending>
