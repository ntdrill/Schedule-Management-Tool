...
- [x] docs(readme): リポジトリルートの README（または無ければ `docs/readme/00_README索引.md`）に `iOS Build & Test` workflow の status badge を追加。マークダウン形式で `https://github.com/ntdrill/Schedule-Management-Tool/actions/workflows/ios-build.yml/badge.svg?branch=sdk-migration` を埋め込み、Actions 一覧へのリンクも併記する — 2026-04-28 02:30 JST commit:(本 push)
